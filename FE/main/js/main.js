...
function updateSurgingTitle() {
  const surgingTitle = document.querySelector(
    '.container-left .box-top-title, .carousel-surging-slide .box-top-title'
  );
  if (surgingTitle) {
    const title = isMobile() ? 'Top 5 Surging' : 'Top 5 Surging';
...
function updateCarouselTitle(slideIndex) {
  const titleElement = document.querySelector(
    '.container-right .box-top-title'
  );
  if (titleElement) {
    if (isMobile()) {
      // 모바일: 3개 슬라이드 - Top 3 Coin, Top 3 Surging, Advertisement
      const titles = ['Top 2 Coin', 'Top 5 Surging', 'Advertisement'];
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (banner) {
    banner.addEventListener('click', () => {
      window.open(
        'https://eggsinthetray.avgmax.team/eggsinthetray/login',
        '_blank'
      );
    });
  }
  ...
  searchForms.forEach(form => {
    form.addEventListener('submit', function (e) {
      ...
      const keyword = this.querySelector('.search-input')
        .value.trim()
        .toLowerCase();
      ...
      const filtered = globalCoins.filter(
        coin =>
          coin.coin_name.toLowerCase().includes(keyword) ||
          coin.user_name.toLowerCase().includes(keyword)
      );
...
function moveSurgingIfMobile() {
  ...
  if (isMobile()) {
    ...
    if (carousel.contains(surgingContainer)) {
      ...
      // 빈 화면 슬라이드가 없다면 추가
      const existingEmptySlide = carousel.querySelector(
  ...
  coins.forEach((coin, idx) => {
    ...
    entry.innerHTML = `<div class="entry-left">
            ...
            <div class="entry-right">
                <span class="entry-change ${upOrDown}">${Math.abs(
      coin.fluctuation_rate.toFixed(2)
    )}%</span>
            </div>`;
...
function renderPriceCoins(coins) {
  ...
  // 모바일에서는 3개만 보여주기
  const displayCoins = isMobile() ? coins.slice(0, 2) : coins;
            ...
            <div class="card-bot">
                ...
                <div class="card-price">
                    <div class="card-change ${upOrDown}">
                        ${Math.abs(
                          coin.change_price.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })
                        )}
                        <span class="change-percent">(${Math.abs(
                          coin.fluctuation_rate.toFixed(2)
                        )}%)</span>
                    </div>
                    <div class="price-icon">
                        ...
                        <span class="price-amount">${coin.current_price.toLocaleString(
                          undefined,
                          { maximumFractionDigits: 0 }
                        )}</span>
                    </div>
...
function renderCardViewCoins(coins) {
  ...
  coins.sort((a, b) => {
    switch (sortKey) {
      ...
      case 'rate':
        return b.fluctuation_rate - a.fluctuation_rate;
      case 'createdAt':
      default:
    ...
    card.innerHTML = `<div class="card-profile">
                ...
                <div class="card-info">
                    ...
                    <div class="info-change ${upOrDown}">
                        <span class="change-amount">${Math.abs(
                          coin.change_price.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })
                        )}</span>
...
function renderListViewCoins(coins) {
  ...
  coins.forEach(coin => {
    ...
    table.innerHTML = `<div class="col col-coin">${coin.coin_name}</div>
            <div class="col col-name">${coin.user_name}</div>
            <div class="col col-price">${coin.current_price.toLocaleString(
              undefined,
              { maximumFractionDigits: 0 }
            )}</div>
            <div class="col col-diff ${upOrDown}">${Math.abs(
      coin.change_price.toLocaleString(undefined, { maximumFractionDigits: 0 })
    )}</div>
            <div class="col col-rate ${upOrDown}">${Math.abs(
      coin.fluctuation_rate
    ).toFixed(2)}%</div>
            <div class="col col-volume">${coin.trade_volume.toLocaleString(
              undefined,
              { maximumFractionDigits: 0 }
            )}</div>
            <div class="col col-low">${coin.lowest_price.toLocaleString(
              undefined,
              { maximumFractionDigits: 0 }
            )}</div>
            <div class="col col-high">${coin.highest_price.toLocaleString(
              undefined,
              { maximumFractionDigits: 0 }
            )}</div>`;
